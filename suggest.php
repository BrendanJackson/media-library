...
if ($_SERVER["REQUEST_METHOD"] == "POST"){ //trim() checks for any whitespace at the beginning of  a field EX. trim($_POST["name"]) 
	$name = trim(filter_input(INPUT_POST,'name',FILTER_SANITIZE_STRING)); //filter_input filters submissions we don't want.
	...
	if (!isset($error_message) && !$mail->ValidateAddress($email)){ //checks if email address is valid, returns true if it is.
	$error_message ="Invalid email address";
	
	}

	if (!isset($error_message)){
		//collects and displays all fields info???How do I access this from terminal?
		$email_body = ""; 
		...
		$mail->addAddress('bajackson859@gmail.com', 'Brendan');     // Add a recipient
		...
		if(!$mail->send()) { 
		    ...
		    //.= concatenates defined info onto what was defined 1st
...
<div class="section page">
	<div class="wrapper">	
		...
		<?php/* to maintain old form data if user fills it out incorrectly use EX. if (isset($name)){echo $name; } */ ?>   
			<table>
				<tr>
        ...
        ?>
				...
				<tr style="display:none"> <!-- Spam filter field -->
